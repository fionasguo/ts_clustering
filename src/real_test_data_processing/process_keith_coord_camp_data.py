...
#     # logging.info(f"{n} aggregated. control shape={control.shape}, treated shape={treated.shape}")
...
#     # control['tweet_time'] = pd.to_datetime(control['tweet_time'],utc=True,format='mixed')
#     # logging.info(f"control min date:{control['tweet_time'].min()}, max date:{control['tweet_time'].max()}")
#     # treated['tweet_time'] = pd.to_datetime(treated['tweet_time'],utc=True,format='mixed')
#     # logging.info(f"treated min date:{treated['tweet_time'].min()}, max date:{treated['tweet_time'].max()}")

#     # control = control[(control['tweet_time']>=pd.Timestamp(start_date,tz='utc')) & (control['tweet_time']<=pd.Timestamp(end_date,tz='utc'))]
#     # treated = treated[(treated['tweet_time']>=pd.Timestamp(start_date,tz='utc')) & (treated['tweet_time']<=pd.Timestamp(end_date,tz='utc'))]
#     # logging.info(f"data {start_date} to {end_date} {agg_time_period}: control shape={control.shape}, treated shape={treated.shape}")

#     # control.to_csv(data_dir+n+'/all_controls.csv',index=False)
#     # treated.to_csv(data_dir+n+'/all_drivers.csv',index=False)

#     control = pd.read_csv(path + 'all_controls.csv', lineterminator='\n')

#     # split
#     chunk_size = 2000000
#     n_chunks = math.ceil(len(control)/chunk_size)
#     for i in range(n_chunks):
#         control[i*chunk_size:(i+1)*chunk_size].to_csv(path+f'all_controls_{i}.csv',index=False)
...
#     #                 'in_reply_to_userid', 'is_retweet', 'retweet_tweetid', 'retweet_userid', 'retweet_username', 
#     #                 'hashtags','urls']
...
#     # treated = pd.concat([control,treated],axis=0)

#     # treated.to_csv(data_dir+n+'/all_controls_drivers_to_valeria.csv',index=False)
#     # logging.info(f"{n} saved.")
...
data_dir = '/nas/eclairnas01/users/siyiguo/hashed_infoOps/'+n+'/'
...
#     all_embeddings = np.vstack((all_embeddings,pickle.load(open(f,'rb'))))
# logging.info(f'loaded saved bert embeddings, shape: {all_embeddings.shape}')
# pickle.dump(all_embeddings,data_dir+'xlmt_embeddings.pkl')
# idx = np.random.randint(len(all_embeddings), size=len(all_embeddings)//2)
# all_embeddings = all_embeddings[idx,:]
# std_scaler = StandardScaler()
# std_scaler = std_scaler.fit(all_embeddings)
...
logging.info(f"PCA embeddings loaded {all_embeddings.shape}.")

df[list(range(n_comp))] = all_embeddings

"""
egyptuae: '2017-01-01' to '2019-08-05', 3D
venezuela: '2020-01-01' to '2021-06-03' 3D
cnhu: '2019-04-21' to '2021-04-05' 3D
"""
...
logging.info(f"raw user embedding ts data - shape: {user_ts_data.shape}, num users={len(pd.unique(df['userid']))}")
...
pickle.dump(ts_array[:,:,:5], open(data_dir+'shorter/xlmt_embeddings_ts_data.pkl','wb'))
logging.info('finished saving xlmt_embedding ts data')
pickle.dump(ts_array[:,:,-1], open(data_dir+'shorter/activity_ts_data.pkl','wb'))
...
pickle.dump(user_links_array, open(data_dir+'shorter/links_data.pkl','wb'))
...
pickle.dump(demo_data,open(data_dir+'shorter/demo_data.pkl','wb'))
...
pickle.dump(gt_data,open(data_dir+'shorter/gt_data.pkl','wb'))