...
user_tot_tweet_threshold = 10 # top 8% users
...
# logging.info(f"min date:{df['date'].min()}, max date:{df['date'].max()}")
...
data_dir = '/nas/eclairnas01/users/siyiguo/rvw_data/'
filename = 'autonomy_health'
logging.info(filename)

cols = ['id', 'text', 'date', 'author.username', 'political_gen']

df = pickle.load(open(data_dir+'health.pkl','rb'))[cols] #'religion.pkl' # 'health.pkl'
df1 = pickle.load(open(data_dir+'autonomy.pkl','rb'))[cols] # 'fetal_rights.pkl' # 'autonomy.pkl'
df = pd.concat([df,df1],axis=0)
df = df.rename(columns={'political_gen':'issue_political_gen'})
logging.info(f"raw {filename} data shape={df.shape}")

target_users = pd.read_csv(data_dir+'gt_data.csv')
df = df.merge(target_users[['author.username','political_gen','elite_label']],how='inner',on='author.username')
logging.info(f"find users in the original rvw analysis: df shape={df.shape}")
logging.info(f"political score disagreement: {len(df[df['political_gen']!=df['issue_political_gen']])}")
logging.info(f"nans:\n{df.isnull().sum()}")
...
user_ts_count = df.groupby(['author.username',pd.Grouper(freq=agg_time_period,key='date')])['id'].count()
user_ts_count_ = user_ts_count.reindex(pd.MultiIndex.from_product([user_ts_count.index.levels[0],entire_time_range],names=['author.username','date']),fill_value=0)
...
# pickle.dump(all_embeddings,open(data_dir+f'bert_embeddings_{filename}.pkl','wb'))
...
# all_embeddings = reducer.fit_transform(all_embeddings)
# pickle.dump(all_embeddings,open(data_dir+f'bert_embeddings_{filename}_pca.pkl','wb'))
# logging.info('PCA results saved')
...
all_embeddings = pickle.load(open(data_dir+f'bert_embeddings_{filename}_pca.pkl','rb'))
df[list(range(n_comp))] = all_embeddings

user_ts_data = df.groupby(['author.username',pd.Grouper(freq=agg_time_period,key='date')])[list(range(n_comp))].sum()
logging.info(f'raw user embedding ts data - shape: {user_ts_data.shape}')
# fill the time series with the entire time range
user_ts_data = user_ts_data.reindex(pd.MultiIndex.from_product([user_ts_data.index.levels[0],entire_time_range],names=['author.username','date']),fill_value=0)
...
# pickle.dump(ts_array, open(data_dir+f'bert_embeddings_ts_data_{filename}.pkl','wb'))
...
user_ts_data_before = user_ts_data[(user_ts_data.index.get_level_values(level=1)>=pd.Timestamp('2022-01-01',tz='utc'))&(user_ts_data.index.get_level_values(level=1)<pd.Timestamp('2022-05-02',tz='utc'))]
logging.info(f'user ts data in selected time range 2022-01-01 to 2022-05-02 - shape: {user_ts_data_before.shape}; number of users: {len(pd.unique(user_ts_data_before.index.get_level_values(level=0)))}, len of entire time range: {len(pd.unique(user_ts_data_before.index.get_level_values(level=1)))}')
ts_array_before = np.array(user_ts_data_before.groupby(level=0).apply(lambda x: x.values.tolist()).tolist())
logging.info(f'shape of np array for the ts data: {ts_array_before.shape}, mean of embeddings: {np.mean(ts_array_before,axis=1)}')
# pickle.dump(ts_array, open(data_dir+f'bert_embeddings_ts_data_{filename}_0101_0502.pkl','wb'))

# user_ts_data_ = user_ts_data[(user_ts_data.index.get_level_values(level=1)>=pd.Timestamp('2022-05-03',tz='utc'))&(user_ts_data.index.get_level_values(level=1)<pd.Timestamp('2022-06-24',tz='utc'))]
# logging.info(f'user ts data in selected time range 2022-05-03 to 2022-06-24 - shape: {user_ts_data_.shape}; number of users: {len(pd.unique(user_ts_data_.index.get_level_values(level=0)))}, len of entire time range: {len(pd.unique(user_ts_data_.index.get_level_values(level=1)))}')
# ts_array = np.array(user_ts_data_.groupby(level=0).apply(lambda x: x.values.tolist()).tolist())
# logging.info(f'shape of np array for the ts data: {ts_array.shape}, mean of embeddings: {np.mean(ts_array,axis=1)}')
# pickle.dump(ts_array, open(data_dir+f'bert_embeddings_ts_data_{filename}_0503_0624.pkl','wb'))

user_ts_data_after = user_ts_data[(user_ts_data.index.get_level_values(level=1)>=pd.Timestamp('2022-06-24',tz='utc'))&(user_ts_data.index.get_level_values(level=1)<pd.Timestamp('2022-11-08',tz='utc'))]
logging.info(f'user ts data in selected time range 2022-06-24 to 2022-11-08 - shape: {user_ts_data_after.shape}; number of users: {len(pd.unique(user_ts_data_after.index.get_level_values(level=0)))}, len of entire time range: {len(pd.unique(user_ts_data_after.index.get_level_values(level=1)))}')
ts_array_after = np.array(user_ts_data_after.groupby(level=0).apply(lambda x: x.values.tolist()).tolist())
logging.info(f'shape of np array for the ts data: {ts_array_after.shape}, mean of embeddings: {np.mean(ts_array_after,axis=1)}')
# pickle.dump(ts_array, open(data_dir+f'bert_embeddings_ts_data_{filename}_0624_1108.pkl','wb'))
...
# # additional_missing_users = ['1mir_r', '247Catholic', '45Teresa22', '77HERCULES77', '9Lovebugs', 'AMC_APE_LFG', 'Agathon_B', 'AmyCampbellUSA', 'Astec57Fred', 'AutisticMumTo3', 'Auwkwaard', 'BIDENsKrakDealr', 'BMoreloving102', 'BabaYaga726', 'Bees3Three', 'BlameCCP', 'Blueskyflowers9', 'BordeleauCody', 'Budisimanjunta', 'CVLovesPosies', 'Camalama800', 'Captainshat69', 'Captive2theWord', 'CatholicCoinDad', 'CoVet_81', 'ConallGulban', 'CuriousJohnnyC', 'Curtashiism', 'D1J3S', 'D_In_Canada', 'DariHijaDeJesus', 'DeSantisFacist', 'Debbiemojitos0', 'Destiny15_FL', 'DocJeremmanuel', 'Eagle1948', 'EllisAppleV', 'Elonzo_Musk', 'Elumin_8', 'EssieJae_jones', 'FISM_Editor', 'GSheralee', 'GartersGunsNGod', 'GioOrSmtgh', 'GodFamily_USA', 'GregCohen29', 'Gregnus_2023', 'GuerrierEst', 'Gwenhwy_fawr', 'HaydeedoNamoNa', 'HeliogenDerek', 'HistorysLight', 'Honeycomb_Craft', 'HortonHearsTx', 'ImArr_m', 'JBSWAGGYSOLDIER', 'JCFUSMC', 'JackieLNic', 'JaeLynn0812', 'JffStep', 'Joel213Woman', 'JossaDosBichos', 'JudicialCorrup3', 'JunimoTweets', 'JunkratTweets', 'KATENOB06816243', 'KDallas1988', 'KatarinyaMariya', 'Kelly4Arizona', 'Kristi_4_Ohio', 'LOVEandCATS4GOP', 'LadyKnightFox', 'LauraTaggart58', 'LgdyasLgddil', 'LindseyHGreen', 'Lisahudsonchow7', 'LivingForGod21', 'LizzieReformed', 'LnzyHou', 'Longshanks708', 'LoriAnnArt', 'LoveOnlyLove__', 'MFreeman_ATASED', 'MNRick6', 'MWC1847', 'M__Bingo_', 'Madd8Moxxie', 'MarysPencil', 'MasiaTarazona', 'MegaMagaMan_', 'MelissafromtheV', 'Menarechildren', 'Mestiza_TK', 'Michael4Ranger', 'MiguelsMother', 'Mormon4Freedom', 'MunazzaKhala', 'Neff68', 'NeromemOtcha', 'NewTexicanDem', 'NightSpringsFM', 'OKMerryMary', 'OKSeanM', 'Official_IAGB', 'OlohanCatherine', 'OnARoll_Winning', 'OrganBots', 'Paul_B_Tovey', 'PostRoeFolks', 'ProLibertyNurse', 'ProbeComicsCol', 'RainLovesYeshua', 'RavenLoveFox', 'RestrictedNews', 'Ricardo86167919', 'RoeYorVote', 'RogueSquadron22', 'RosannaM1970', 'RosarioMacri6', 'Rotofreddit', 'SAndrew8433', 'SSStrays', 'Sageone007', 'Sam_C_Fraser', 'SamuelLabadie2', 'Santas1stLady', 'Scott_4Trump', 'SeaDweller1665', 'Servantoftruth_', 'ShellyForce1', 'SiPapiLoSiento', 'SirMalik61', 'Skmyspace', 'Sock_Collector', 'Squirts_Momma', 'Sue0707077', 'Supercarcat13', 'SupraOmniEst', 'TBD_2022', 'TFauneAlfo', 'TerriSueMonark', 'Th3OwlLady', 'TheJamesRossi', 'TheKaiaPeiris', 'TheStoriedWitch', 'The_Heathan', 'ThrivinginJesus', 'Tinky729', 'TomPaineCS', 'TrumpWonbigely', 'Truth1938', 'USMCJEFF60', 'VictorRaj4Life', 'WCristoRex', 'WhitePetalPoet', 'WokeSlayerr', 'adamosoftsreal', 'ajlb1492', 'allie_wde', 'andreisa12345', 'antiabort_memes', 'arquCervant', 'astorcaeli', 'bblock29', 'bearrunswyld', 'bethlastname', 'black_sheep__13', 'blairtony897', 'bluegirlcharjo', 'brigittbrien', 'carlblomgren', 'cbritt130', 'chainalpha_io', 'daniel201112955', 'darkraven19', 'eatityo', 'ejswensson', 'elizbethlmaness', 'faithuntoworks', 'fatherLukasz', 'fighthetyranny', 'funkycatholicus', 'genzmom5', 'hiitslissy', 'hookskat', 'imthatboii11', 'info_blooket', 'jadedandamused', 'jan6committee', 'jaycecarson1', 'jdadair2728', 'jennlunaflora', 'jmc_blue', 'joe172726kag', 'joemckosky', 'kathy_craft', 'kathy_sherrill', 'kaylajohnson28', 'kingojungle', 'kung_fu_jedi', 'lastbubblerises', 'lisabelleknoll', 'londoner_woman', 'lysistrata327', 'm_chevygirl1969', 'mandyhanf', 'mat_raymond3', 'melcat1776', 'melinda4you', 'minniemomm', 'mrjohnnybronco', 'mrontsuj', 'muddymothball', 'nancyhashope', 'neumalbernardo', 'newsallyb', 'nm_scout', 'nocturnnina', 'nononsenseFL', 'notifisayso', 'ohhmystars_', 'oldwick1e', 'peacepipeexhale', 'pedenlangford93', 'perptualjade', 'popo66v', 'psworldwide2023', 'pxpist', 'realwifey22', 'resist_cynicism', 'rftchumn', 'rolin3674', 'sarnib05', 'sharona321', 'sherrystanley', 'shubes11667', 'sidikokat', 'skatelyn7', 'summerof1951', 'sunrisegaocldy', 'suzyinthesun_s', 'tazmilly_', 'tfinnaz', 'thomist79', 'tito_edwards', 'tris_themom', 'trytbfaithful', 'twofeksgiven', 'uncomfytruth38', 'veinycock_', 'witchy__wife', 'wyatteich']
...
# missing_index = list(ordered_user_index.difference(demo.index))
# logging.info(f"demo data num users={len(demo)}, our data num usrs={len(ordered_user_index)}, missing users in demo data: len={len(missing_index)}")
# for u in missing_index:
#     demo.loc[u,demo_cols] = 100.0

# demo = demo.loc[ordered_user_index,demo_cols]
# demo = demo.astype(float)
# demo[np.isnan(demo)] = 100.0
# demo = demo.astype(float)
# logging.info(f"demo data shape={demo.shape}, nans={np.isnan(demo).sum()}")
# pickle.dump(demo.values,open(data_dir+f'demo_data_{filename}.pkl','wb'))

# ####################### ground truth data - political ideology ########################
mask = (np.mean(ts_array_before,axis=(1,2))!=0) & (np.mean(ts_array_after,axis=(1,2))!=0)

# get ground truth data
gt = df.groupby('author.username')[['political_gen','elite_label']].first().loc[ordered_user_index,] # 0 = left, 1 = right
logging.info(f"raw gt shape={gt.shape}")
gt = gt[mask]
logging.info(f"gt shape={gt.shape}, num left users={len(gt[gt.political_gen==0])}, num right users={len(gt[gt.political_gen==1])}")
gt = gt.reset_index()
elites_in_issue = list(gt[~gt['elite_label'].isnull()].index)
logging.info(f"indices of elites in this issue {filename}:\n{elites_in_issue}")
logging.info(f"len={len(elites_in_issue)},left elites={len(gt[gt['elite_label']==0])}, right elites={len(gt[gt['elite_label']==1])}")
# pickle.dump(gt.values,open(data_dir+f'gt_data_{filename}.pkl','wb'))
# logging.info('finished saving ground truth data')

"""
Then filter for users that have tweets both before and after!!!!!!
"""
...
"""
Index(['id', 'author.username', 'date', 'text', 'entities.urls',
       'entities.hashtags', 'lang', 'source', 'author.id', 'author.date',
