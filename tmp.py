...
data_dir = '/nas/eclairnas01/users/siyiguo/ts_clustering/test_rvw_autonomy_health/'
...
gt = pickle.load(open('/nas/eclairnas01/users/siyiguo/rvw_data/gt_data_autonomy_health.pkl','rb'))
gt = gt.astype(bool)
...
# KNN
def knn(K):
    knn_before = np.argpartition(-before,K,axis=1)[:,:K]
    p_right_neighbors_before = np.sum(np.isin(knn_before,right_users),axis=1)/K
    p_left_neighbors_before = 1 - p_right_neighbors_before
    knn_after = np.argpartition(-after,K,axis=1)[:,:K]
    p_right_neighbors_after = np.sum(np.isin(knn_after,right_users),axis=1)/K
    p_left_neighbors_after = 1 - p_right_neighbors_after
    logging.info(f"\nnum right users={len(right_users)}, knn_before shape={knn_before.shape}, p_right_neighbors_before shape={p_right_neighbors_before.shape}\n{p_right_neighbors_before[:10]}")
